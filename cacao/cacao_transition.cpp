...
)   :   tmpl_before( new cacao::Template( "treq_before", tmpl_before_str, Template::Traversion ) )
    ,   tmpl_refactoring( new cacao::Template( "treq_refactoring", tmpl_refactoring_str, Template::Traversion ) )
{}
...
std::string Transition::transform( const std::string str
    ...
    cacao::Template* transformation_tmpl
            = new cacao::Template( "trf_" + name, equiv->transformation( stack ), Template::CacaoTemplate );
