...
//    using cmap_t::contains;
    ...
    Stack( const std::string stype, const Config& configuration = *new Config() )
    ...
    content_type* get( const std::string key ) {
        if ( ! this->contains(key) ) return nullptr;
        for ( typename cmap_t::iterator it = cmap_t::begin()
            ; it != cmap_t::end(); ++it
        ) {
            if ( it->first.compare(key) == 0 ) return it->second;
        }
        return nullptr;
    }

    const content_type* get( const std::string key ) const {
        if ( ! this->contains(key) ) return nullptr;
        for ( typename cmap_t::iterator cit = cmap_t::cbegin()
            ; cit != cmap_t::cend(); ++cit
        ) {
            if ( cit->first.compare(key) == 0 ) return cit->second;
        }
        return nullptr;
    }
};
